...
public class ReseauMetro {
	...
	public List<Station> trouverItineraireLePlusRapide(Station depart, Station arrivee) {
		...
		while (!aVisiter.isEmpty()) {
			...
			System.out.println("Station courante : " + stationActuelle.getNom() + "; "
					+ stationActuelle.getVoies().size() + " voies");
	...
	public static void trouverCheminOptimalBellman(Station stationDepart, Station stationArrivee) {
	...
	public static void trouverCheminOptimalInter(Station stationDepart, Station stationIntermediaire,
		...
		// Calcul du premier trajet entre la station de départ et la station
		// intermédiaire
		trouverCheminOptimalBellman(stationDepart, stationIntermediaire);
		...
		// Calcul du deuxième trajet entre la station intermédiaire et la station
		// d'arrivée
		trouverCheminOptimalBellman(stationIntermediaire, stationArrivee);
	...
	private static List<String> determineLignesParcourues(List<Station> stations) {
		...
		for (int i = 0; i < stations.size() - 1; i++) {
			Station stationCourante = stations.get(i);
			Station stationSuivante = stations.get(i + 1);

			for (Ligne ligne : lignes) {
				for (Voie voie : ligne.getVoies()) {
					if (voie.getStationDepart().equals(stationCourante)
							&& voie.getStationArrivee().equals(stationSuivante)) {
						lignesParcourues.add(ligne.getNom());
						break;
					}
				}
			}
...
// -------------------Méthode Trajet Intermediaire----------------//
	...
	public List<Voie> passerParUneStation(double longitudeUtil, double latitudeUtil, double longitudeDestination,
		...
		for (Ligne ligne : lignes) {
			...
			List<Voie> voiesParcourus = trouverVoiesEntreStations(ligne, stationDepartPlusProche, stationIntermediaire);
		...
		// ------------Chemin Optimal 2 de la station Intermédiaire jusqu'à
		// destination-------------//
